 ...
 * @param <type> $n Имя выбираемого поля
 * @param <type> $id Имя первичного ключа
 * @param <type> $w where
 * @return <type>
 */
function db_sel($t,$n = 'name',$id='id',$w='hide=0 and', $o='') {
    if($n=='') $n = 'name';
    if($id=='') $id = 'id';
    if($o=='') $o = $n;
...
/**
 * Удалить запись
 * @param <type> $t     Таблица из которой удалить
 * @param <type> $id    id записи
 * @param <type> $loc   Куда перенаправить после удаления
 * @param <type> $tc    Имя таблица, в которой проверять наличие зависимости
 * @param <type> $cc    Имя колонки в которой проверять наличие зависимости
 * @return <type>
 */
